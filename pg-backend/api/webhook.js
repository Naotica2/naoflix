...
// Supabase client minimalis (tanpa SDK)
async function updateSupabase(transactionRef, userId, durationDays) {
  ...
  // 2. Set profile is_vip = true dan masa berlakunya
...
export default async function handler(req, res) {
  ...
  // Hams PG merekomendasikan verifikasi signature
  ...
  if (type === 'deposit' && status === 'SUCCESS') {
    ...
    try {
      // Kita harus tahu userId untuk transaksi ini. Ambil dari Supabase:
      const supabaseUrl = process.env.SUPABASE_URL;
      const serviceKey = process.env.SUPABASE_SERVICE_KEY;
      ...
      // Kembalikan 200 OK agar Hams PG berhenti mengirim ulang webhook
