  ...
  return (
    <View style={[styles.msgRow, isMe && styles.msgRowMe]}>
      ...
      <View style={[styles.msgContent, isMe && styles.msgContentMe]}>
        {!isMe && (
          <Text style={[styles.msgUser, { color: isDark ? '#8ab4f8' : '#1a73e8' }]}>
            {msg.username}
          </Text>
        )}
        <View style={[styles.msgBubble, isMe ? styles.msgBubbleMe : styles.msgBubbleOther]}>
          <Text style={[styles.msgText, emojiOnly && styles.msgEmojiOnly]}>
            {msg.text}
          </Text>
        </View>
...
export default function ChatForum() {
  ...
  // Don't render if not logged in or on immersive screen
  if (!user || !profile || isHiddenScreen) return null;
          ...
          <Reanimated.View
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
              ...
              <View style={[styles.panelHeader, { borderBottomColor: isDark ? '#333' : '#eee' }]}>
                <Icon name="comments" size={16} color={isDark ? '#8ab4f8' : '#3b82f6'} />
                <Text style={[styles.panelTitle, { color: isDark ? '#eee' : '#222' }]}>Forum Chat</Text>
                <TouchableOpacity onPress={togglePanel} style={styles.closeBtn}>
                  <Icon name="times" size={18} color={isDark ? '#888' : '#666'} />
                </TouchableOpacity>
              </View>
