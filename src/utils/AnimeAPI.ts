...
class AnimeAPI {
  ...
  static async home(signal?: AbortSignal): Promise<EpisodeBaruHome> {
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.home(signal).catch(() => ({ newAnime: [], jadwalAnime: {} })), 8000, { newAnime: [], jadwalAnime: {} }));
  ...
  static async newAnime(
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.latestAnime(page || 1, signal).catch(() => []), 8000, []));
  ...
  static async getAnimeByGenre(
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.getAnimeByGenre(genre, page, signal).catch(() => []), 8000, []));
  ...
  static async search(query: string, signal?: AbortSignal): Promise<SearchAnime> {
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.search(query, signal).catch(() => ({ result: [] })), 8000, { result: [] }));
  ...
  static async searchMovies(page: number = 1, signal?: AbortSignal): Promise<SearchAnime> {
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.searchMovies(page, signal).catch(() => ({ result: [] })), 8000, { result: [] }));
  ...
  static async fromUrl(
    ...
    // Animelovers detail page: /anime/slug/ (no /episode/)
    if (link.includes('api.fruatre.my.id') && link.includes('/anime/') && !link.includes('/episode/')) {
    ...
    // Animelovers episode page: /anime/slug/episode/postId
    if (link.includes('api.fruatre.my.id') && link.includes('/episode/')) {
  ...
  static async listAnime(
    ...
    if (isSourceActive(prefs, 'animelovers')) promises.push(withTimeout(animelovers.animeList(signal).catch(() => []), 8000, []));
  ...
  static async jadwalAnime(signal?: AbortSignal) {
    ...
    if (isSourceActive(prefs, 'animelovers')) {
      promises.push(withTimeout(animelovers.jadwalAnime(signal).catch(() => ({})), 8000, {}));
    }
