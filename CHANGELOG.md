...
## [v2.1.0] - Social Features & Watch Party Polish
### Added
- **Direct Messaging (DM) & Chat Requests:** Menambahkan fitur Private Chat antar pengguna. Termasuk sistem *Preview-Before-Accept* di mana kamu bisa mengintip isi pesan dari orang tak dikenal di menu *Requests* sebelum memutuskan untuk menerima atau menolak undangannya.
- **Discord-Style Rich Presence:** Menambahkan status aktivitas *real-time* pada profil pengguna! Pengguna lain kini bisa melihat apa yang sedang kamu tonton (contoh: "Sedang menonton One Piece") langsung dari menu profilmu.
- **Markdown & Link Support in Chat:** Ruang obrolan DM dan Forum kini mendukung format teks tebal (*bold* dengan `**`), miring (*italic* dengan `_`), dan deteksi *link* otomatis untuk pengalaman mengobrol yang lebih ekspresif.
- **Watch Party (Nobar):** Fitur utama baru! Nonton bareng teman atau orang lain secara *real-time* jarak jauh. Tersedia *live chat* in-room, sinkronisasi *playback* otomatis, dan undangan Nobar (Invite) yang terintegrasi dengan DM.
- **Watch Party (Nobar) Lag Tolerance:** Menambahkan toleransi *disconnect* 15 detik pada mode Nobar. Jika koneksi internetmu terputus sebentar, kamu tidak akan langsung terlempar keluar dari *room*. Jika sang Host terputus permanen, *room* akan otomatis dibubarkan dengan aman.
- **Watch Party Auto-Pause Sync:** Pemutar video Nobar kini akan otomatis dalam status *Pause* setiap kali Host berganti ke episode baru (Next/Prev Episode). Hal ini mencegah Host menonton mendahului Guest yang masih dalam proses *loading*.
- **Anilist Cover Fallback System:** Menambahkan sistem pencadangan *thumbnail* pintar (GraphQL) yang akan mencari dan menarik poster anime langsung dari *database* Anilist apabila server sumber mengalami pemblokiran gambar (403 Forbidden).
- **GIF Avatar & Banner Support:** Semua pengguna kini dapat menggunakan *Avatar* dan *Banner* berformat GIF animasi di profil mereka untuk mengekspresikan diri secara lebih hidup!

### Changed
- **Profile Revamp:** Tampilan halaman Profil kini dirombak total! UI menjadi lebih modern dan interaktif untuk mengakomodasi fitur sosial baru seperti melihat status teman dan mengelola DM.
- **Settings Overhaul:** Menu Pengaturan (*Settings*) telah diperbarui dan dirapikan, memberikanmu pengalaman navigasi opsi yang jauh lebih intuitif.
- **Smart Image Loading Engine:** Memperbarui arsitektur sistem pemuat gambar (`ImageLoading` & Fresco) dengan menyuntikkan *Header* khusus (`Referer`) dan *User-Agent* desktop (Chrome Windows) untuk menembus proteksi Cloudflare/Anti-Hotlink pada sumber-sumber berat seperti MyAnimeList.
- **AI Assistant Persona:** Sistem *AI Assistant* NaoFlix (jangan berharap lebih pada ai ini ya)
- **Dynamic Title Propagation:** Mengoptimalkan penyaluran data judul anime dari *Hero Carousel* dan *ListAnimeComponent* ke komponen gambar agar sistem *Fallback* Anilist memiliki akurasi pencarian tinggi.

### Fixed
- **Animelovers Fix:** Fix animelovers.
- **WatchHistoryGrid Crash:** Memperbaiki *bug ReferenceError* pada komponen *WatchHistoryGrid* (Riwayat Tontonan) yang sebelumnya dapat membuat aplikasi *crash* akibat kesalahan destrukturisasi properti dari *React Navigation*.
- **TV Series Episode Loading:** Memperbaiki masalah pada daftar *Season* dan Episode Film (Series) yang sebelumnya gagal memuat (*empty list*) di halaman Detail. Penarikan data kini dikonfigurasi ulang menggunakan integrasi *Token/JWT* yang tepat agar seluruh episode dapat terbuka secara instan.
